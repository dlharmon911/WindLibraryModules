...
namespace wind
{
	...
	map_t::map_t(ALLEGRO::VECTOR_2D<size_t> size) : m_data(std::make_shared<map_t::element_type[]>(size.get_x() * size.get_y())), m_size(size) {}
	...
	auto map_t::reset(size_t width, size_t height) -> bool
	{
		...
		if (this->m_data)
		{
			this->m_size = ALLEGRO::VECTOR_2D(width, height);
		}

		return (bool)this->m_data;
	}

	auto map_t::reset(const ALLEGRO::VECTOR_2D<size_t>& size) -> bool
	{
		return this->reset(size.get_x(), size.get_y());
	}
	...
	auto map_t::size() const -> const ALLEGRO::VECTOR_2D<size_t>&
	{
		return this->m_size;
	}
	...
	{
		ALLEGRO::ASSERT(index < (this->m_size.get_x() * this->m_size.get_y()));
	...
	{
		return iterator(this->m_data, this->m_size.get_x() * this->m_size.get_y());
	}
	...
	{
		return const_iterator(this->m_data, this->m_size.get_x() * this->m_size.get_y());
	}
}
